...
class BlockMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        down_proj = torch.matmul(up_proj, self.down_proj.transpose(1, 2))
        down_proj = torch.sum((down_proj * router_score.T.unsqueeze(-1)), dim=0)
...
class MoEExperts(nn.Module):
    def __init__(self, config: MiniCPMConfig):
        ...
        self.intermediate_size = config.intermediate_size
        self.num_experts = config.num_experts
        self.up_proj = MoELinear(self.hidden_size, self.intermediate_size, self.num_experts)
        self.down_proj = MoELinear(self.intermediate_size, self.hidden_size, self.num_experts)
        self.act_fn = ACT2FN[config.hidden_act]
...
class MoEMLP(nn.Module):
    def __init__(self, config: MiniCPMConfig, layer_idx: int):
        ...
        elif self.routing_strategy == "topp":
